...
function serializeGraph(owner) {
  ...
  if (!owner)
    return {};
  return __spreadValues(__spreadValues({}, serializeValues(owner.sourceMap)), owner.owned ? serializeChildren(owner) : {});
}
...
function serializeChildren(root) {
  ...
  for (let i = 0, len = root.owned.length; i < len; i++) {
    ...
    result[node.componentName ? `${node.componentName}:${node.name}` : node.name] = __spreadValues(__spreadValues({}, serializeValues(node.sourceMap)), node.owned ? serializeChildren(node) : {});
  }
  return result;
}
function getSymbol() {
...
//# sourceMappingURL=chunk-NCSU6M7M.js.map
